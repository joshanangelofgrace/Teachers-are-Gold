...
post_install do |installer|
  ...
  if Dir.exists?('Teachers-are-Gold/Resources/Settings.bundle') && File.exists?(acknowledgements_plist)
    FileUtils.cp(acknowledgements_plist, 'Teachers-are-Gold/Resources/Settings.bundle/Acknowledgements.plist')
  end

  environment_file = 'Pods/Target Support Files/Pods/Pods-environment.h'
  if File.exists?(environment_file)
    FileUtils.cp(environment_file, 'Teachers-are-Gold/Other-Sources/Pods-Environment.h')
  end
end
